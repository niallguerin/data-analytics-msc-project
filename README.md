...
In general, the feature engineering scripts are run in any order but last one is always the merge script. The IR module uses gensim so that it has a number of dictionary, corpus, and other matrix files that get constructed once run. They are not uploaded here as they are large files. Likewise the /data directory is not uploaded here for any of the pipeline directories as they are generally too large. Once the ML models are trained and loaded, test queries are fired and these generate various resultset files in a format the ML models can ingest. If they produce resultsets similar to the input query then the ML model has a better chance of making a best answer prediction from a useful resultset. The goal of this tool is filtering not just running the models over the SO dataset hence the IR module component too earlier in the pipeline as it is linked to another project I was working on in the past for my own use. A text snippet with URL link is provide in the command-line console as final step. This was a prototype successor application to  the older information retrieval and curated note-taking system which leveraged the Google CSE API - vendelligence. Gensim was used to prototype removing the reliance on the Google CSE API.
